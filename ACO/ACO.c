...
int main(){
    srand(time(NULL));
    int dist_static [12][12] = {
        { 0 , 29 , 20 , 21 , 16 , 31 , 100 , 12 , 4 , 31 , 18 , 26 } ,
        { 29 , 0 , 15 , 29 , 28 , 40 , 72 , 21 , 29 , 41 , 35 , 22 } ,
        { 20 , 15 , 0 , 15 , 14 , 25 , 81 , 9 , 23 , 27 , 19 , 17 } ,
        { 21 , 29 , 15 , 0 , 4 , 12 , 92 , 12 , 25 , 13 , 23 , 18 } ,
        { 16 , 28 , 14 , 4 , 0 , 16 , 94 , 9 , 20 , 16 , 22 , 15 } ,
        { 31 , 40 , 25 , 12 , 16 , 0 , 95 , 24 , 36 , 3 , 28 , 21 } ,
        { 100 , 72 , 81 , 92 , 94 , 95 , 0 , 90 , 101 , 99 , 88 , 85 } ,
        { 12 , 21 , 9 , 12 , 9 , 24 , 90 , 0 , 15 , 25 , 14 , 11 } ,
        { 4 , 29 , 23 , 25 , 20 , 36 , 101 , 15 , 0 , 35 , 19 , 28 } ,
        { 31 , 41 , 27 , 13 , 16 , 3 , 99 , 25 , 35 , 0 , 30 , 24 } ,
        { 18 , 35 , 19 , 23 , 22 , 28 , 88 , 14 , 19 , 30 , 0 , 13 } ,
        { 26 , 22 , 17 , 18 , 15 , 21 , 85 , 11 , 28 , 24 , 13 , 0 }
        };

    int n = sizeof(dist_static) / sizeof(dist_static[0]);
    int **dist = createIntMatrix(n);
    for(int i = 0 ; i < n ; i++){        for(int j = 0 ; j < n ; j++){
            dist[i][j] = dist_static[i][j];
            }
        }
    ...
    Ant best_ant = AOC_tsp(dist,n,max_iteration,alpha,beta,Q);

    printf("Chemin Optimale : \n");
    for(int i = 0 ; i < n ; i++){
        printf("%d", best_ant.path[i]);
        printf("->");
    }
    printf("%d",best_ant.path[0]);
    printf("\n");
    printf("Cout totale : %d",best_ant.total_cost);

    freeAnt(&best_ant);
    freeMatrix((void**) dist, n);
